...
public class WeaponsManagerScript : MonoBehaviour
{
    public List<WeaponScript> weaponList = new List<WeaponScript>(); // List of available weapons.
    public List<WeaponScript> weaponEquipped = new List<WeaponScript>(); // List of weapons equipped.

    public int maxWeapons; // Maximum number of weapons the player can carry.

    public PlayerControllerScript pcs; // The player.
    public PlayerStatusController psc;

    public Camera mainCamera; // Player camera.
    public PlayerUIScript UI; // UI.
    ...
    private void Update()
    {
        ...
        canSwitch = ready && pcs.moveState != MoveState.Running && !isSwitching && !pcs.isClimbing;
    ...
    private void SearchForWeapons()
    {
        ...
        if (Physics.Raycast(ray, out hitInfo, interactionRange))
        {
            ...
            if (hitInfo.collider.GetComponent<WeaponInfo>() != null)
            {
                ...
                WeaponScript w = GetWeapon(hitInfo.collider.GetComponent<WeaponInfoScript>().weaponId);

                // If the weapon exists and the player is not running.
                if (w != null && pcs.moveState != MoveState.Running)
                {
                    // If the weapon is not already equipped.
                    if (!isEquipped(w._WeaponId))
                    {
                        ...
                        if (weaponEquipped.Count < maxWeapons)
                        {
                            ...
                            UI.ShowWeaponPickUp(!isEquipped(w._WeaponId), w._WeaponName);
                        ...
                        else // If you do not have space to get new weapons, then we should change the current weapon.
                        {
                            ...
                            UI.ShowWeaponSwitch(true, weaponEquipped[currentWeapon]._WeaponName, w._WeaponName);

                            // If press the pick up button.
                            if (Input.GetKeyDown(KeyCode.E) && !isSwitching && ready)
                            {
                                // Saves the selected weapon, to be instantiated your copy after.
                                WeaponScript lastWeapon = weaponEquipped[currentWeapon];
                    ...
                    else
                    {
                        ...
                        UI.ShowAmmoInfo(w._WeaponName + " ALREADY EQUIPPED");
                    }
                }
            }
            else
    ...
    /// </summary>
    private void PickAndDrop(GameObject w, WeaponScript lastWeapon)
    {
        //yield return new WaitForSeconds(weaponEquipped[currentWeapon].GetHideTime());

        // Instantiate a clone of the current weapon.
        Instantiate(lastWeapon._DroppablePrefab, w.transform.position, w.transform.rotation);
    ...
    /// </summary>
    private WeaponScript GetWeapon(int id)
    {
    ...
    /// </summary>
    private int GetWeaponIndex(WeaponScript w)
    {
        for (int i = 0; i < weaponEquipped.Count; i++)
        {
            if (w._WeaponId == weaponEquipped[i]._WeaponId)
            {
                return i;
            }
        }
    ...
    private bool isEquipped(int id)
    {
        for (int i = 0; i < weaponEquipped.Count; i++)
        {
            if (weaponEquipped[i]._WeaponId == id)
            {
                return true;
            }
        }
    ...
    private void CalculateWeight()
    {
        ...
        for (int i = 0; i < weaponEquipped.Count; i++)
        {
            weight += weaponEquipped[i]._Weight; // Sum weight of all weapons.
        }
        psc._Weigth = Mathf.RoundToInt(weight); // Convert to nearest integer and sets the weight the player are carrying.
        //pcs._Weigth = Mathf.RoundToInt(weight); // Convert to nearest integer and sets the weight the player are carrying.
    }
    ...
    /// </summary>
	private void Select(WeaponScript w)
    {
        w.Select();
    }
    ...
    /// </summary>
    private IEnumerator Switch(WeaponScript oldWeapon, WeaponScript newWeapon)
    {
        isSwitching = true;
        oldWeapon.Deselect();
