...
{
    public AudioManagerScript audioManager; // The audio manager.
    public CharacterControlScript _Character; // Player.

    public MotionBlurScript motionBlur; // The motion blur effect.
    public VignetteAndChromaticAberrationScript vignetteAndChromaticAberration; // The camera Vignette and Chromatic Aberration.
    public CameraMotionBlurScript cameraMotionBlur; // The camera motion blur effect.
    ...
    private IEnumerator Disoriented()
    {
        if (!disoriented) // The coroutine is not running?
        {
            ...
            audioManager.PlayNoiseSound(noiseSound, disorientIntensity);    //PlayNoiseSound(noiseSound, disorientIntensity);
